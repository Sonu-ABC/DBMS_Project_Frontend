...
def _tab_health_overview(pid, patient_name):
    ...
    if active_alerts:
        critical = sum(1 for a in active_alerts if a.get("severity_level") == "Critical")
        high = sum(1 for a in active_alerts if a.get("severity_level") == "High")
        medium = sum(1 for a in active_alerts if a.get("severity_level") == "Medium")
        low = sum(1 for a in active_alerts if a.get("severity_level") == "Low")
        ...
        st.info(f"💡 You have **{len(active_alerts)}** active alert(s). Go to the **My Alerts** tab for details.")
    else:
